...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
    if (isset($_COOKIE['id']))
    {
        ...
        else
        {
            ...
            $Previous_date = date('Y-m-d H:i:s', strtotime($today_date . ' -35 days'));
            $plan_care = mysql_query("SELECT * FROM sp_detailed_event_plan_of_care WHERE end_date <'$time' AND professional_vender_id='$professional_vender_id' AND Session_status =3  ");
            $row = mysql_num_rows($plan_care);
            if ($row > 0)
            {
                $Loc_query = mysql_query("UPDATE sp_detailed_event_plan_of_care SET Session_status=1  WHERE end_date <'$time' AND professional_vender_id='$professional_vender_id' AND Session_status =3  ");
            ...
            foreach ($status as $key => $status)
            {
                ...
                if ($status == 0)
                {
                    ...
                    $plan_of_cares = mysql_query("SELECT * FROM sp_detailed_event_plan_of_care where (Actual_Service_date >='$fromDate%' AND Actual_Service_date <='$toDate%') AND professional_vender_id='$professional_vender_id' AND status=1  ");
                    $num_row = mysql_num_rows($plan_of_cares);

                    $plan_of_care = mysql_query("SELECT * FROM sp_detailed_event_plan_of_care where (Actual_Service_date >='$fromDate%' AND Actual_Service_date <='$toDate%') AND professional_vender_id='$professional_vender_id' AND status=1  ORDER BY start_date DESC LIMIT $begin, $pageSize  ");
                ...
                else
                {
                    ...
                    $plan_of_cares = mysql_query("SELECT * FROM sp_detailed_event_plan_of_care where (Actual_Service_date >='$fromDate%' AND Actual_Service_date<='$toDate%') AND professional_vender_id='$professional_vender_id'AND Session_status ='$status' AND status=1  ");
                    $num_row = mysql_num_rows($plan_of_cares);

                    $plan_of_care = mysql_query("SELECT * FROM sp_detailed_event_plan_of_care where (Actual_Service_date >='$fromDate%' AND Actual_Service_date<='$toDate%') AND professional_vender_id='$professional_vender_id'AND Session_status ='$status' AND status=1  ORDER BY start_date DESC LIMIT $begin, $pageSize  ");
                    $num_rows = mysql_num_rows($plan_of_care);
                    $pages = ceil($num_row / $pageSize);
