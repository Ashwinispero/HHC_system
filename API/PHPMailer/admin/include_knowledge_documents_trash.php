...
else
{
    ...
    if($_POST['sort_order'] !='' && ($_POST['sort_field']=='title' || $_POST['sort_field']=='added_date'))
    {
   ...
   //print_r($recArgs);
    ...
    {      
        echo '<div class="table-responsive"><table class="table table-hover table-bordered">
                <tr> 
                    <th><a href="javascript:void(0);" onclick="changePagination(\'KnowledgeDocsTrashListing\',\'include_knowledge_documents_trash.php\',\'\',\'\',\''.$order.'\',\'title\');" style="color:#00cfcb;">Title'.' '.$img1.'</a></th>
                    <th width="11%">Document File</th>
                    <th width="10%"><a href="javascript:void(0);" onclick="changePagination(\'KnowledgeDocsTrashListing\',\'include_knowledge_documents_trash.php\',\'\',\'\',\''.$order.'\',\'added_date\');" style="color:#00cfcb;">Added Date '.' '.$img2.'</a></th>   
        ...
        {  
           $knowledge_document_id=$recListValue['document_id']; 
            echo '<tr id="KnowledgeDocRecord_'.$knowledge_document_id.'">
                    <td>'.$recListValue['title'].'</td>
                    <td>';
                    if(!empty($recListValue['document_file'])) 
                    {          
                       echo '<div style="padding-top:2px;margin-left:18px;"><img alt="Download" src="images/icon-pdf25.png"><br/><a title="" href="downloadFiles.php?type=KnowledgeDocumentFile&knowledge_document_id='.base64_encode($knowledge_document_id).'">Download</a></div>';
                    }
                    echo '</td>
                    ...
                    echo '<td>
                              <ul class="actionlist">
                                <li><a href="javascript:void(0);" onclick="return change_status('.$knowledge_document_id.','.$recListValue['isDelStatus'].',\'Revert\');" data-toggle="tooltip" title="Revert"><img src="images/revert.png"  alt="Revert"></a></li>       
                                <li><a href="javascript:void(0);" onclick="return change_status('.$knowledge_document_id.','.$recListValue['status'].',\'CompleteDelete\');" data-toggle="tooltip" title="Delete"><img src="images/icon-delete.png"  alt="Delete"></a></li>';       
                        echo '</ul></td>
                  </tr>';
        }
        echo '</table></div>';
    ...
    if($paginationCount)
    {
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="pagination-dropdown">
                                <label>
                                    <select name="show_records" onchange="changePagination(\'KnowledgeDocsTrashListing\',\'include_knowledge_documents_trash.php\',\'\',this.value,\''.$order1.'\',\''.$sort_variable.'\')">';                            
        ...
        if($pageId>1)
        {
            echo '
                <td style="width:10px;"></td>
                <td align="right" onclick="changePagination(\'KnowledgeDocsTrashListing\',\'include_knowledge_documents_trash.php\',\''.($pageId-1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" valign="middle"><input type="button" class="btn btn-prev" value="<"></td>';
        }
        else
        {
            echo '
        ...
        if($pageId!=($paginationCount))
        {
            echo '
                ...
                <td valign="middle"><input onclick="changePagination(\'KnowledgeDocsTrashListing\',\'include_knowledge_documents_trash.php\',\''.($pageId+1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" type="button" class="btn btn-next" value=">"></td>';
        }
        else
        {
            echo '
