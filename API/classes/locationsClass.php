...
class locationsClass extends AbstractDB 
{
    ...
    public function API_Locations($args)
	{
		 $insertData['professional_service_id']=$this->escape($args['professional_service_id']);
        $insertData['Name']=$this->escape($args['Name']);
        
	   $RecordId=$this->query_insert('sp_professional_location',$insertData);
	   		$last_id =mysql_insert_id();
	}
	
 public function API_LocationsDetails($args)
	{
		 $insertData['lattitude']=$this->escape($args['lattitude']);
        $insertData['longitude']=$this->escape($args['longitude']);
		 $insertData['professional_location_id']=$this->escape($args['professional_location_id']);
		 $insertData['location_name']=$this->escape($args['location_name']);
        
	   $RecordId=$this->query_insert('sp_professional_location_details',$insertData);
	   		
	}
    public function API_LogLocationsDetails($args)
	{
			$date = date('Y-m-d H:i:s');
	  $insertData['Added_date']=$date;
		 $insertData['Session_id']=$this->escape($args['Session_id']);
		 $insertData['event_by_professional']=$this->escape($args['event_by_professional']);
        $insertData['professional_id']=$this->escape($args['professional_id']);
		 $insertData['latitude']=$this->escape($args['latitude']);
        $insertData['longitude']=$this->escape($args['longitude']);
		
        
	   $RecordId=$this->query_insert('sp_log_location_for_session',$insertData);
	   		
	}
	public function APILogevent($args)
	{
	 $date = date('Y-m-d H:i:s');
	  $insertData['Added_date']=$date;
		 $insertData['Role_id']=$this->escape($args['Role_id']);
		 $insertData['professional_id']=$this->escape($args['professional_id']);
        $insertData['logged_from']=$this->escape($args['logged_from']);
		 $insertData['logStatement']=$this->escape($args['logStatement']);
       
		
        
	   $RecordId=$this->query_insert('sp_log_for_event',$insertData);
