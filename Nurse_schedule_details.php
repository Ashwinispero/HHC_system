...
<script type="text/javascript">
	function Nurse_Scheduling_dw()
	{
		//alert('ashwini');
		window.open('csv_download_Nurse_scheduling.php?flag=1','_self','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=800,height=600,directories=no,location=no'); 
		return false
	}
	function Previous_date_Nurse()
	{
		var Previous_date_Nurse=document.getElementById('Previous_date_Nurse').value;
		var xmlhttp;
		...
		xmlhttp.onreadystatechange = function()
		{
            if(xmlhttp.readyState==4 && xmlhttp.status==200)
			{
                ...
                document.getElementById("Next_date_report_Nurse").innerHTML=xmlhttp.responseText;
			}
		}
		xmlhttp.open("POST","Previous_day_report_Nurse.php?Previous_date="+Previous_date_Nurse,true);
		xmlhttp.send();
	}
	function Next_date_Nurse()
	{
		
		var Next_date_Nurse=document.getElementById('Next_date_Nurse').value;
		//alert(Next_date_Nurse);
		var xmlhttp;
		if(window.XMLHttpRequest)
		{
			xmlhttp=new XMLHttpRequest();
		...
		xmlhttp.open("POST","Next_day_report_Nurse.php?Next_date="+Next_date_Nurse,true);
		xmlhttp.send();
	}
	function Nurse_Profesional_Name_Search(Professional_id_Nurse)
	{
		//var Professional_id=document.getElementById('Professional_id_Nurse').value;
		var Professional_id_old=$('option[value="'+Professional_id_Nurse+'"]');
		var Professional_id_Nurse = (Professional_id_old.attr('id'));
		var xmlhttp;
		if(window.XMLHttpRequest)
		{
			xmlhttp=new XMLHttpRequest();
		...
		xmlhttp.open("POST","Nurse_professional_search.php?Professional_id="+Professional_id_Nurse,true);
		xmlhttp.send();
	}
	</script>
    <body style="background-color: #00cfcb;">
          <div class="row">
				...
				<div class="col-lg-3 paddingR0 pull-LEFT text-LEFT dropdown">
					...
					<input type="text" id="Professional_id_Nurse" onchange="Nurse_Profesional_Name_Search(this.value)" list="Professional_Nurse" placeholder="Enter Professional Name" style="width:100%">
				...
				<?php 
	$HCA = mysql_query("SELECT * FROM `sp_professional_services` where service_id='5' and status='1' ORDER BY service_professional_id  ASC");
	...
	if($row_count > 0)
		...
		date_default_timezone_set('Asia/Kolkata'); 
			...
			$END_Date = date('Y-m-d H:i:s', strtotime($new_date . ' +1 days'));
