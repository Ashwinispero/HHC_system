...
<script type="text/javascript">
	function HCA_Scheduling_dw()
	{
		window.open('csv_download_HCA_scheduling.php?flag=1','_self','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=800,height=600,directories=no,location=no'); 
		return false
	}
	function Next_date_other()
	{
		...
		xmlhttp.onreadystatechange = function()
		{
            if(xmlhttp.readyState==4 && xmlhttp.status==200)
			{
                ...
                document.getElementById("Next_date_report_other").innerHTML=xmlhttp.responseText;
			}
		}
		xmlhttp.open("POST","Other_Next_day_report.php?Next_date="+Next_date,true);
		xmlhttp.send();
		
	}
	function Previous_date_other()
	{
		...
		xmlhttp.onreadystatechange = function()
		{
            if(xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				alert('Shows Privous 7 Days');
                document.getElementById("Next_date_report_other").innerHTML=xmlhttp.responseText;
			}
		}
		xmlhttp.open("POST","Other_Previous_day_report.php?Previous_date="+Previous_date,true);
		xmlhttp.send();
	}
	function Other_services_Name_Search(Professional_id)
	{
		...
		xmlhttp.open("POST","Other_services_search.php?Professional_id="+Professional_id,true);
		xmlhttp.send();
    ...
    <body style="background-color: #00cfcb;">
          <div class="row">
				...
				<div class="panel-heading" align="center" style="color:#00cfcb">
                    <h4>Schedule For Other Services</h4>
                </div>
				<br>
				 <!--<div class="col-lg-2 paddingR0 pull-right text-right dropdown">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-original-title="Download Report" onclick="return HCA_Scheduling_dw();">
                            <img src="images/download.png" border="0" class="example-fade" />                                
                        </a>
                    </div>-->
			<div id="Next_date_report_other">	
				<div class="col-lg-2 paddingR0 pull-LEFT text-LEFT dropdown">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-original-title="Back" onclick="return Previous_date_other();">
				...
				<div class="col-lg-3 paddingR0 pull-LEFT text-LEFT dropdown">
					...
					<input type="text" id="Professional_id" onchange="Other_services_Name_Search(this.value)" list="Professional_HCA" placeholder="Enter Professional Name" style="width:100%">
					<datalist id="Professional_HCA" style="overflow-y: auto!important">
						<?php
						$Query=mysql_query("SELECT * FROM `sp_professional_services` where service_id!='4' AND service_id!='3' AND service_id!='2' AND service_id!='5' AND service_id!='13' AND service_id!='16' and status='1' ORDER BY service_professional_id  ASC");
						...
						if($row_count > 0)
						{
							while ($Professional_id = mysql_fetch_array($Query))
							{
								...
								else
								{
									...
									//$Job_type=$Professional_name['Job_type'];
									?>
				...
				<div class="col-lg-2 paddingR0 pull-right text-right dropdown">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-original-title="Next" onclick="return Next_date_other();">
				...
				<?php 
	$HCA = mysql_query("SELECT * FROM `sp_professional_services` where service_id!='4' AND service_id!='3' AND service_id!='2' AND service_id!='5' AND service_id!='13' and status='1' AND service_id!='16' ORDER BY service_professional_id  ASC");
