...
<script type="text/javascript">
	function Physician_Scheduling_dw()
	{
		//alert('ashwini');
		window.open('csv_download_Physician_scheduling.php?flag=1','_self','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=800,height=600,directories=no,location=no'); 
		return false
	}
	function Next_date_Physician()
	{
		var Next_date_Physician=document.getElementById('Next_date_Physician').value;
		//alert(Next_date_Physician);
		var xmlhttp;
		...
		xmlhttp.onreadystatechange = function()
		{
            if(xmlhttp.readyState==4 && xmlhttp.status==200)
			{
                ...
                document.getElementById("Next_date_report_Physician").innerHTML=xmlhttp.responseText;
			}
		}
		xmlhttp.open("POST","Next_day_report_Physician.php?Next_date="+Next_date_Physician,true);
		xmlhttp.send();
	}
	function Previous_date_Physician()
	{
		var Previous_date_Physician=document.getElementById('Previous_date_Physician').value;
		//alert(Previous_date_Physician);
		var xmlhttp;
		if(window.XMLHttpRequest)
		{
			xmlhttp=new XMLHttpRequest();
		...
		xmlhttp.open("POST","Previous_day_report_Physician.php?Previous_date="+Previous_date_Physician,true);
		xmlhttp.send();
	}
	function Physician_Profesional_Name_Search(Professional_id_physician)
	{
		var Professional_id_old=$('option[value="'+Professional_id_physician+'"]');
		var Professional_id_physician = (Professional_id_old.attr('id'));
		//var Professional_id_physician=document.getElementById('Professional_id_physician').value;
		var xmlhttp;
		if(window.XMLHttpRequest)
		{
			xmlhttp=new XMLHttpRequest();
		...
		xmlhttp.open("POST","physician_professional_search.php?Professional_id="+Professional_id_physician,true);
		xmlhttp.send();
	}
	</script>
    <body style="background-color: #00cfcb;">
          <div class="row">
				...
				<div class="panel-heading" align="center" style="color:#00cfcb">
                    <h4>Schedule Physician Assistant</h4>
                </div>
				<!--<div class="col-lg-2 paddingR0 pull-right text-right dropdown">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-original-title="Download Report" onclick="return Physician_Scheduling_dw();">
                            <img src="images/download.png" border="0" class="example-fade" />                                
                        </a>
                    </div>-->
					<br>
					<div id="Next_date_report_Physician">
					<div class="col-lg-2 paddingR0 pull-LEFT text-LEFT dropdown">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-original-title="Back" onclick="return Previous_date_Physician();">
				...
				<div class="col-lg-3 paddingR0 pull-LEFT text-LEFT dropdown">
					...
					<input type="text" id="Professional_id_physician" onchange="Physician_Profesional_Name_Search(this.value)" list="Professional_Physician" placeholder="Enter Professional Name" style="width:100%">
					<datalist id="Professional_Physician">
						<?php
						$Query=mysql_query("SELECT * FROM `sp_professional_services` where service_id='2' and status='1' ORDER BY service_professional_id  ASC");
				...
				<div class="col-lg-2 paddingR0 pull-right text-right dropdown">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-original-title="Next" onclick="return Next_date_Physician();">
				...
				<?php 
	$HCA = mysql_query("SELECT * FROM `sp_professional_services` where service_id='2' and status='1' ");
	...
	if($row_count > 0)
		...
		?>
	<input style="display:none" id="Next_date_Physician" value="<?php echo $date6;?>"></input>
	<input style="display:none" id="Previous_date_Physician" value="<?php echo $date;?>"></input>
  <thead>
    <tr>
      <th style="width:11%">Physician Assistant Name</th>
