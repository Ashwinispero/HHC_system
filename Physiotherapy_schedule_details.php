...
<head>
	...
	<title>Spero HealthCare</title>
	<link href="css/modern-business.css" rel="stylesheet">
	<script src="js/bootstrap.min.js"></script>
</head>
<script type="text/javascript">
	function Physiotherapy_Scheduling_dw()
	{
		//alert('ashwini');
		window.open('csv_download_Physiotherapy_scheduling.php?flag=1','_self','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=800,height=600,directories=no,location=no'); 
		return false
	}
	function Next_date_Physiotherapy()
	{
		var Next_date_Physiotherapy=document.getElementById('Next_date_Physiotherapy').value;
		var xmlhttp;
		...
		xmlhttp.onreadystatechange = function()
		{
			...
			{
				alert('Shows Next 7 Days');
                document.getElementById("Next_date_report_Physiotherapy").innerHTML=xmlhttp.responseText;
			}
		}
		xmlhttp.open("POST","Next_day_report_Physiotherapy.php?Next_date="+Next_date_Physiotherapy,true);
		xmlhttp.send();
	}
	function Previous_date_Physiotherapy()
	{
		var Previous_date_Physiotherapy=document.getElementById('Previous_date_Physiotherapy').value;
		//alert(Previous_date_Physiotherapy);
		var xmlhttp;
		if(window.XMLHttpRequest)
		{
			xmlhttp=new XMLHttpRequest();
		...
		xmlhttp.onreadystatechange = function()
		{
            if(xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				...
				alert('Shows Previous 7 Days');
                document.getElementById("Next_date_report_Physiotherapy").innerHTML=xmlhttp.responseText;
			}
		}
		xmlhttp.open("POST","Previous_day_report_Physiotherapy.php?Previous_date="+Previous_date_Physiotherapy,true);
		xmlhttp.send();
	}
	function Physio_Profesional_Name_Search(Professional_id_physician)
	{
		//var Professional_id_physio=document.getElementById('Professional_id_physio').value;
		var Professional_id_old=$('option[value="'+Professional_id_physician+'"]');
		var Professional_id_physio = (Professional_id_old.attr('id'));
		var xmlhttp;
		if(window.XMLHttpRequest)
		{
			xmlhttp=new XMLHttpRequest();
		...
		xmlhttp.open("POST","physiotherapy_professional_search.php?Professional_id="+Professional_id_physio,true);
		xmlhttp.send();
	}
	</script>
    <body style="background-color: #00cfcb;">
          <div class="row">
				...
				<div class="panel-heading" align="center" style="color:#00cfcb">
                    <h4>Schedule For Physiotherapy</h4>
                </div>
				<br>
				<div id="Next_date_report_Physiotherapy">
				<!--<div class="col-lg-2 paddingR0 pull-right text-right dropdown">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-original-title="Download Report" onclick="return Physiotherapy_Scheduling_dw();">
                            <img src="images/download.png" border="0" class="example-fade" />                                
                        </a>
                    </div>-->
					<div class="col-lg-2 paddingR0 pull-LEFT text-LEFT dropdown">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-original-title="Back" onclick="return Previous_date_Physiotherapy();">
				...
				<div class="col-lg-3 paddingR0 pull-LEFT text-LEFT dropdown">
					...
					<input type="text" id="Professional_id_physio" onchange="Physio_Profesional_Name_Search(this.value);" list="Professional_Physio" placeholder="Enter Professional Name" style="width:100%">
					<datalist id="Professional_Physio" style="overflow-y: scroll;">
						<?php
						$Query=mysql_query("SELECT * FROM `sp_professional_services` where service_id='16' and status='1' ORDER BY service_professional_id  ASC");
				...
				<div class="col-lg-2 paddingR0 pull-right text-right dropdown">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-original-title="Next" onclick="return Next_date_Physiotherapy();">
				...
				<?php 
	$HCA = mysql_query("SELECT * FROM `sp_professional_services` where service_id='16' and status='1' ORDER BY service_professional_id  ASC");
	...
	if($row_count > 0)
		...
		date_default_timezone_set('Asia/Kolkata'); 
			...
			$Added_date = date('Y-m-d H:i:s', strtotime($new_date . ' -30 days'));
		...
		?>
	<input style="display:none" id="Next_date_Physiotherapy" value="<?php echo $date6;?>"></input>
	<input style="display:none" id="Previous_date_Physiotherapy" value="<?php echo $date;?>"></input>
  <thead>
    <tr>
      <th style="width:11%">Physiotherapy Name</th>
