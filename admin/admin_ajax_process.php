...
<?php
    if($_REQUEST['action']=='DoLogin')
    {
        ...
        $checkExist = "SELECT admin_user_id, first_name, name, email_id,password,type,status FROM sp_admin_users WHERE email_id ='".$UserName."' AND password ='".$Password."'";      
        ...
        if(mysql_num_rows($db->query($checkExist)))
        {
            ...
            if($adminLog['status']=='1')
            {
                ...
                $_SESSION['admin_user_name'] = $adminLog['first_name'] . " " . $adminLog['name'];

                // Getting assigned hosptial details
                $arg = array();
                $arg['admin_user_id'] = $_SESSION['admin_user_id'];
                $assignedHospitalDtls = $adminuserClass->getAssignedHospitalById($arg);
                $assignedHospitalids = array();
                foreach ($assignedHospitalDtls AS  $valAssignedHospitalids) {
                    $assignedHospitalids[] = $valAssignedHospitalids['hospital_id'];
                }

                if (!empty($assignedHospitalids)) {
                    $_SESSION['assigned_hospital_ids'] = $assignedHospitalids;
                }
                unset($arg);
           ...
           <div>
               ...
               <form class="form-inline" name="change_admin_password" id="frm_change_admin_password" method="post"  action ="admin_ajax_process.php?action=SubmitChangePasswordForm" autocomplete="off">
                 <div class="editform">
                     ...
                     <div class="value">
                         <input type="hidden" name="admin_id" id="admin_id" value="<?php if(!empty($modifyadmin['admin_user_id'])) { echo $modifyadmin['admin_user_id']; } ?>" />
                         <input type="password" name="admin_old_password" id="admin_old_password" value="<?php if(!empty($_POST['admin_old_password'])) { echo $_POST['admin_old_password']; } else { echo ""; } ?>" class="validate[required,maxSize[25]] form-control" maxlength="25" />
                     </div>
                </div>
                <div class="editform">
                     <label>New Password <span class="required">*</span></label>
                     <div class="value">
                         <input type="password" name="admin_new_password" id="admin_new_password" value="<?php if(!empty($_POST['admin_new_password'])) { echo $_POST['admin_new_password']; } else { echo ""; } ?>" class="validate[required,custom[passwordVal],minSize[8],maxSize[25]] form-control" maxlength="25" />
                         <div id="pwd_strength">
                         </div>
                     </div>
                </div>
                <div class="editform">
                     ...
                     <div class="value">
                         <input type="password" name="admin_confirm_password" id="admin_confirm_password" value="<?php if(!empty($_POST['admin_confirm_password'])) { echo $_POST['admin_confirm_password']; } else { echo ""; } ?>" class="validate[required,custom[passwordVal],minSize[8],equals[admin_new_password],maxSize[25]] form-control" maxlength="25" />
                     </div>
                </div>
                 <div class="modal-footer">
