...
$date2=date_create("$toDate");
 ...
 $total=0;
	...
	{
		$payments = mysql_query("SELECT * FROM sp_payments where status=1 and date_time BETWEEN '$formDate1%' AND '$toDate2%' ORDER BY date_time ASC");
	}
	else
	{
		$payments = mysql_query("SELECT * FROM sp_payments where status=1 ORDER BY date_time ASC");
    ...
    if($row_count > 0)
    {
        $datas .=  '<table cellpadding="1" cellspacing="1" align="left" border="1" id="mainTableBg">
			...
			$Voucher_Ref=$hhc_code.'/'.$event_code;				
             ...
             $datas .= '<td>'.$branch.'/'.$finasial_year.'-'.$finasial_year1.'/'.$payment_receipt_no_voucher_no.'</td>';
			$datas .= '<td>'.$date.'</td>';
			$datas .= '<td>'.$branch.'/'.$finasial_year.'-'.$finasial_year1.'/'.$bill_no_ref_no.'</td>';
			$datas .= '<td>'.$first_name.' '.$middle_name.' '.$name.'</td>';
			
			$datas .= '<td>'.$finalcost.'</td>';
			$datas .= '<td>'.$professional_name.'</td>';
			$datas .= '<td>'.$bank_cash.'</td>';
			$datas .= '<td>'.$payment_rows['cheque_DD__NEFT_no'].'</td>';
			$datas .= '<td>'.$cheque_DD__NEFT_date.'</td>';
			$datas .= '<td>'.$payment_rows['party_bank_name'].'</td>';
			$datas .= '<td>'.$Voucher_Ref.' - '.$payment_rows['Comments'].'</td>';
