...
$recArgs = $_SESSION['physiotherapy_unit_calculation_list_args'];
...
if ($recListCount)
{
    ...
    foreach($recList as $recListKey => $recListValue)
    {
        ...
        $datas .= '<td>'.$recListValue['totalUnits'].'</td>';
        $datas .= '</tr>';
    }
} else {
...
$db->close();    
//echo $csv;
$filepath="CSV/".time()."physiotherapyUnitReport.xls";
//echo $filepath;exit;
$file=fopen($filepath,"w");
fwrite($file,$datas);
fclose($file);
header("Content-Disposition: attachment; filename=physiotherapyUnitList_".date("Y-m-d").".xls");
