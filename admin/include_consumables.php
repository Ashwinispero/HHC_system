...
else
{
    ...
    {
        $recArgs['filter_name']= "consumable_id";
        $recArgs['filter_type']= "DESC";
    }
    //var_dump($recArgs);
   //print_r($recArgs);
    $_SESSION['consumable_list_args'] = $recArgs;
    $recListResponse= $consumablesClass->ConsumablesList($recArgs);
    ...
    {      
        echo '<div class="table-responsive"><table class="table table-hover table-bordered">
                <tr> 
                    <th><a href="javascript:void(0);" onclick="changePagination(\'ConsumablessListing\',\'include_consumables.php\',\'\',\'\',\''.$order.'\',\'name\');" style="color:#00cfcb;">Name '.' '.$img1.'</a></th>
                    <th width="10%"><a href="javascript:void(0);" onclick="changePagination(\'ConsumablessListing\',\'include_consumables.php\',\'\',\'\',\''.$order.'\',\'type\');" style="color:#00cfcb;">Type '.' '.$img2.'</a></th>
                    <th><a href="javascript:void(0);" onclick="changePagination(\'ConsumablessListing\',\'include_consumables.php\',\'\',\'\',\''.$order.'\',\'manufacture_name\');" style="color:#00cfcb;">Manufacture '.' '.$img3.'</a></th>
                    <th width="10%"><a href="javascript:void(0);" onclick="changePagination(\'ConsumablessListing\',\'include_consumables.php\',\'\',\'\',\''.$order.'\',\'rate\');" style="color:#00cfcb;">Rate '.' '.$img4.'</a></th>
                    <th width="10%">Added Date</th>
                    <th class="'.$col_class.'">Action</th>
                </tr>';   
        foreach ($recList as $recListKey => $recListValue) 
        {  
           $consumable_id=$recListValue['consumable_id']; 
            echo '<tr id="Record_'.$consumable_id.'">
                    ...
                    echo '<td>
                              <ul class="actionlist">';
                                    if($recListValue['status']=='1')
                                        echo '<li><a href="javascript:void(0);" data-toggle="tooltip" onclick="return change_status('.$consumable_id.','.$recListValue['status'].',\'2\');" title="Active"><img src="images/icon-active.png" alt="Active"></a></li>';
                                    if($recListValue['status']=='2')
                                        echo '<li><a href="javascript:void(0);" data-toggle="tooltip" onclick="return change_status('.$consumable_id.','.$recListValue['status'].',\'1\');" title="Inactive"><img src="images/icon-inactive.png" alt="Inactive"></a></li>'; 
                                
                                    echo '<li><a href="javascript:void(0);" onclick="return consumable_add('.$consumable_id.');" data-toggle="tooltip" title="Edit"><img src="images/icon-edit.png" alt="Edit"></a></li>';
                                    // if($del_visible=='Y') { echo '<li><a href="javascript:void(0);" onclick="return change_status('.$consumable_id.','.$recListValue['status'].',\'3\');" data-toggle="tooltip" title="Delete"><img src="images/icon-delete.png" alt="Delete"></a></li>'; }
    ...
    if($paginationCount)
    {
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="pagination-dropdown">
                                <label>
                                    <select name="show_records" onchange="changePagination(\'ConsumablessListing\',\'include_consumables.php\',\'\',this.value,\''.$order1.'\',\''.$sort_variable.'\')">';                            
        ...
        if($pageId>1)
        {
            echo '
                <td style="width:10px;"></td>
                <td align="right" onclick="changePagination(\'ConsumablessListing\',\'include_consumables.php\',\''.($pageId-1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" valign="middle"><input type="button" class="btn btn-prev" value="<"></td>';
        }
        else
        {
            echo '
        ...
        if($pageId!=($paginationCount))
        {
            echo '
                ...
                <td valign="middle"><input onclick="changePagination(\'ConsumablessListing\',\'include_consumables.php\',\''.($pageId+1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" type="button" class="btn btn-next" value=">"></td>';
        }
        else
        {
            echo '
