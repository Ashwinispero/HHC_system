...
else
{
	...
	if($flag=='2')
			{
		//echo abc;
		$query=mysql_query("SELECT * FROM sp_events  where Tally_Remark='1' ");
		...
		for($i=1; $i<=$row_count;)
			...
			while ($event_rows = mysql_fetch_array($query))
				{		
					$caller_id=$event_rows['caller_id'];
					$Purpose_id=$event_rows['purpose_id'];
					$patient_id=$event_rows['patient_id'];
					$event_id=$event_rows['event_id'];
					$caller_name=mysql_query("SELECT * FROM sp_callers  where caller_id='$caller_id' ");
					$caller_name=mysql_fetch_array($caller_name);
                    $first_name=$caller_name['first_name'];
					$middle_name= $caller_name['middle_name'];
                     $name=$caller_name['name'];
					 
					  $sp_purpose_call=mysql_query("SELECT * FROM sp_purpose_call  where purpose_id='$Purpose_id' ");
					$sp_purpose_call=mysql_fetch_array($sp_purpose_call);
                    $sp_purpose_call=$sp_purpose_call['name'];
					
				
			 echo '<tr>
                    <td>'.$event_rows['event_code'].'</td>
					 
					 <td>'.$first_name." ".$middle_name." ".$name.'</td>
					  <td>'.$sp_purpose_call.'</td>
					   <td>'.date('d M Y',strtotime($recListValue['event_date'])).'</td>
					    <td>'.$event_rows['note'].'</td>
						 <td>'.$event_rows['description'].'</td>
						  <td>'.date('d M Y',strtotime($recListValue['added_date'])).'</td>
					<td>'.$event_rows['status'].'</td>';
                    echo '<td>
                              <ul class="actionlist">
                                <li><a href="manage_event_summary.php?patient_id='.base64_encode($patient_id).'&event_id='.base64_encode($event_id).'" data-toggle="tooltip" title="View Event"><img src="images/icon-view.png"  alt="View Event"></a></li>';
