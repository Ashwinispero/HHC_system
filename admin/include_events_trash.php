   ...
   //print_r($recArgs);
    ...
    {      
        echo '<div class="table-responsive"><table class="table table-hover table-bordered">
                <tr> 
                    <th><a href="javascript:void(0);" onclick="changePagination(\'EventsTrashListing\',\'include_events_trash.php\',\'\',\'\',\''.$order.'\',\'event_code\');" style="color:#00cfcb;">Event Id '.' '.$img1.'</a></th>
                    <th>Caller Name</th>
                    <th>Purpose</th>
                    <th>Event Date</th>
                    <th>Note</th>
        ...
        { 
           $event_id=$recListValue['event_id']; 
            echo '<tr id="EventRecord_'.$event_id.'">
                    ...
                    echo '<td>
                            <ul class="actionlist">
                                <li><a href="javascript:void(0);" onclick="return change_status('.$event_id.','.$recListValue['isDelStatus'].',\'Revert\');" data-toggle="tooltip" title="Revert"><img src="images/revert.png"  alt="Revert"></a></li> 
                                <li><a href="javascript:void(0);" onclick="return change_status('.$event_id.','.$recListValue['status'].',\'CompleteDelete\');" data-toggle="tooltip" title="Delete"><img src="images/icon-delete.png"  alt="Delete"></a></li> 
                            </ul></td>
                  </tr>';
        }
        echo '</table></div>';
    ...
    if($paginationCount)
    {
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="pagination-dropdown">
                                <label>
                                    <select name="show_records" onchange="changePagination(\'EventsTrashListing\',\'include_events_trash.php\',\'\',this.value,\''.$order1.'\',\''.$sort_variable.'\')">';                            
        ...
        if($pageId>1)
        {
            echo '
                <td style="width:10px;"></td>
                <td align="right" onclick="changePagination(\'EventsTrashListing\',\'include_events_trash.php\',\''.($pageId-1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" valign="middle"><input type="button" class="btn btn-prev" value="<"></td>';
        }
        else
        {
            echo '
        ...
        if($pageId!=($paginationCount))
        {
            echo '
                ...
                <td valign="middle"><input onclick="changePagination(\'EventsTrashListing\',\'include_events_trash.php\',\''.($pageId+1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" type="button" class="btn btn-next" value=">"></td>';
        }
        else
        {
            echo '
