   ...
   //print_r($recArgs);
    ...
    {      
        echo '<div class="table-responsive"><table class="table table-hover table-bordered">
                <tr> 
                    <th><a href="javascript:void(0);" onclick="changePagination(\'LocationsTrashListing\',\'include_locations_trash.php\',\'\',\'\',\''.$order.'\',\'location\');" style="color:#00cfcb;">Location  '.' '.$img1.'</a></th>
                    <th width="10%"><a href="javascript:void(0);" onclick="changePagination(\'LocationsTrashListing\',\'include_locations_trash.php\',\'\',\'\',\''.$order.'\',\'pin_code\');" style="color:#00cfcb;">PIN Code'.' '.$img2.'</a></th>
        ...
        { 
           $location_id=$recListValue['location_id']; 
            echo '<tr id="LocationRecord_'.$location_id.'">
                    ...
                    echo '<td>
                              <ul class="actionlist">
                                <li><a href="javascript:void(0);" onclick="return change_status('.$location_id.','.$recListValue['isDelStatus'].',\'Revert\');" data-toggle="tooltip" title="Revert"><img src="images/revert.png"  alt="Revert"></a></li>
                                <li><a href="javascript:void(0);" onclick="return change_status('.$location_id.','.$recListValue['status'].',\'CompleteDelete\');" data-toggle="tooltip" title="Delete"><img src="images/icon-delete.png"  alt="Delete"></a></li>'; 
                        echo '</ul></td>
                  </tr>';
        }
        echo '</table></div>';
    ...
    if($paginationCount)
    {
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="pagination-dropdown">
                                <label>
                                    <select name="show_records" onchange="changePagination(\'LocationsTrashListing\',\'include_locations_trash.php\',\'\',this.value,\''.$order1.'\',\''.$sort_variable.'\')">';                            
        ...
        if($pageId>1)
        {
            echo '
                <td style="width:10px;"></td>
                <td align="right" onclick="changePagination(\'LocationsTrashListing\',\'include_locations_trash.php\',\''.($pageId-1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" valign="middle"><input type="button" class="btn btn-prev" value="<"></td>';
        }
        else
        {
            echo '
        ...
        if($pageId!=($paginationCount))
        {
            echo '
                ...
                <td valign="middle"><input onclick="changePagination(\'LocationsTrashListing\',\'include_locations_trash.php\',\''.($pageId+1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" type="button" class="btn btn-next" value=">"></td>';
        }
        else
        {
            echo '
