...
else
{
    ...
    $_SESSION['physiotherapy_unit_calculation_list_args'] = $recArgs;
    ...
    $recListResponse = $professionalsClass->physiotherapyUnitCalculationList($recArgs);
    ...
    {      
        echo '<div class="table-responsive"><table class="table table-hover table-bordered">
                <tr> 
                    <th><a href="javascript:void(0);" onclick="changePagination(\'physiotherapyUnitCalculationReportListing\',\'include_physiotherapy_unit_calculation_report.php\',\'\',\'\',\''.$order.'\',\'professional_name\');" style="color:#00cfcb;">Prof. Name '.' '.$img1.'</a></th>
                    <th><a href="javascript:void(0);" onclick="changePagination(\'physiotherapyUnitCalculationReportListing\',\'include_physiotherapy_unit_calculation_report.php\',\'\',\'\',\''.$order.'\',\'sp.mobile_no\');" style="color:#00cfcb;">Mobile '.' '.$img2.'</a></th>
                    <th>Unit</th>
					<th>Action</th>
                </tr>';   
        ...
        { 
           ...
           $professionalId = $recListValue['professional_vender_id'];
		   
            echo '<tr id="jobClosureRecord_' . $eventProfessionalId  . '">
					<td>' . $recListValue['professional_name'] . '</td>
                    <td>' . $recListValue['mobile_no'] . '</td>
                    <td>' . $recListValue['totalUnits'] . '</td>
                    <td>
                        <ul class="actionlist">
                            <li>
                                <a href="javascript:void(0);" onclick="return view_physiotherapy_unit_calculation(' . $professionalId . ', \''.$searchfromDate.'\', \''.$searchToDate.'\');" data-toggle="tooltip" title="View Unit"><img src="images/icon-view.png" alt="View Unit"></a>
                            </li>
                        </ul>
                    </td>
                  </tr>';
    ...
    if($paginationCount)
    {
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="pagination-dropdown">
                                <label>
                                    <select name="show_records" onchange="changePagination(\'physiotherapyUnitCalculationReportListing\',\'include_physiotherapy_unit_calculation_report.php\',\'\',this.value,\''.$order1.'\',\''.$sort_variable.'\')">';                            
        ...
        if($pageId>1)
        {
            echo '
                <td style="width:10px;"></td>
                <td align="right" onclick="changePagination(\'physiotherapyUnitCalculationReportListing\',\'include_physiotherapy_unit_calculation_report.php\',\''.($pageId-1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" valign="middle"><input type="button" class="btn btn-prev" value="<"></td>';
        }
        else
        {
            echo '
        ...
        if($pageId!=($paginationCount))
        {
            echo '
                ...
                <td valign="middle"><input onclick="changePagination(\'physiotherapyUnitCalculationReportListing\',\'include_physiotherapy_unit_calculation_report.php\',\''.($pageId+1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" type="button" class="btn btn-next" value=">"></td>';
        }
        else
        {
            echo '
