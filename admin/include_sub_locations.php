...
else
{
    ...
    if($_POST['sort_field']=='location_name')
        $img1 = $img;
    if($_POST['sort_field']=='pin_code')
        $img2 = $img;
   
    if($_POST['sort_order'] !='' && ($_POST['sort_field']=='location_name'))
    {
    ...
    {
        $recArgs['filter_name']= "sub_location_id";
        $recArgs['filter_type']= "DESC";
    }
    //var_dump($recArgs);
   //print_r($recArgs);
    $recListResponse= $locationsClass->SubLocationsList($recArgs);
    ...
    {      
        echo '<table class="table table-hover table-bordered">
                <tr> 
                    ...
                    <th width="15%"><a href="javascript:void(0);" onclick="changePagination(\'SubLocationsListing\',\'include_sub_locations.php\',\'\',\'\',\''.$order.'\',\'location_name\');" style="color:#00cfcb;">Sub Location Name'.' '.$img1.'</a></th>
        ...
        { 
           $sub_location_id=$recListValue['sub_location_id']; 
            echo '<tr id="SubLocationRecord_'.$location_id.'">
                    <td width="15%">'.$recListValue['location'].'</td>
                    <td width="15%">'.$recListValue['location_name'].'</td>
                    <td width="10%">'.date('d M Y',strtotime($recListValue['added_date'])).'</td>
                    <td width="10%">'.$recListValue['statusVal'].'</td>';
                    echo '<td width="15%">
                              <ul class="actionlist">';
                                    if($recListValue['status']=='1')
                                        echo '<li><a href="javascript:void(0);" data-toggle="tooltip" onclick="return change_status('.$sub_location_id.','.$recListValue['status'].',\'Inactive\');" title="Active"><img src="images/icon-active.png"  alt="Active"></a></li>';
                                    if($recListValue['status']=='2')
                                        echo '<li><a href="javascript:void(0);" data-toggle="tooltip" onclick="return change_status('.$sub_location_id.','.$recListValue['status'].',\'Active\');" title="Inactive"><img src="images/icon-inactive.png"  alt="Inactive"></a></li>'; 
                                
                                    echo '<li><a href="javascript:void(0);" onclick="return vw_add_sub_location('.$location_id.','.$sub_location_id.');" data-toggle="tooltip" title="Edit"><img src="images/icon-edit.png" width="22" height="23" alt="Edit"></a></li>
                                         <li><a href="javascript:void(0);" onclick="return change_status('.$sub_location_id.','.$recListValue['status'].',\'Delete\');" data-toggle="tooltip" title="Delete"><img src="images/icon-delete.png"  alt="Delete"></a></li>'; 
                        echo '</ul></td>
                  </tr>';
        }
        echo '</table>';
    ...
    if($paginationCount)
    {
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="pagination-dropdown">
                                <label>
                                    <select name="show_records" onchange="changePagination(\'SubLocationsListing\',\'include_sub_locations.php\',\'\',this.value,\''.$order1.'\',\''.$sort_variable.'\')">';                            
        ...
        if($pageId>1)
        {
            echo '
                <td style="width:10px;"></td>
                <td align="right" onclick="changePagination(\'SubLocationsListing\',\'include_sub_locations.php\',\''.($pageId-1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" valign="middle"><input type="button" class="btn btn-prev" value="<"></td>';
        }
        else
        {
            echo '
        ...
        if($pageId!=($paginationCount))
        {
            echo '
                ...
                <td valign="middle"><input onclick="changePagination(\'SubLocationsListing\',\'include_sub_locations.php\',\''.($pageId+1).'\',\'\',\''.$order1.'\',\''.$sort_variable.'\')" type="button" class="btn btn-next" value=">"></td>';
        }
        else
        {
            echo '
