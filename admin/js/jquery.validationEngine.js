    ...
    var methods = {
        ...
        _validateField: function(field, options, skipAjaxFieldValidation) {
            ...
            if (!isAjaxValidator) {
                if (options.isError)
                    ...
                    $("#unique"+field.attr("id")).remove();
                    //methods._closePrompt(field); ---edited by satish
            }
        ...
        _ajax: function(field, rules, i, options) {
            ...
            if(currentId=="username")
            {
                ...
                if(patt1.test(field.attr("value"))==false)
                {
                    var errorField = $($("#" + currentId)[0]);
                    methods._showPrompt(errorField, '', "pass", true, options);
                    return false;
                }
            }
            ...
            if (!options.isError) {
                $.ajax({
                    ...
                    success: function(json) {
                        ...
                        // make sure we found the element
                        if (errorField.length == 1) {
                            if(currentId=="invitaion_code")
                                $(".invitationCodeAccepted").hide();
                            var status = json[1];
                            ...
                            if (status === false) {
                                ...
                                if(status=="IVCUsed")
                                    var alertTextOk = rule.extraData;
                                else
                                ...
                                if (alertTextOk)
                                    ...
                                    methods._closePrompt(errorField);
                                if(currentId=="invitaion_code" && status!="IVCUsed")
                                    $(".invitationCodeAccepted").show();
                                //alert(field.attr("id"));
                            }
                        }
                    }
        ...
        _buildPrompt: function(field, promptText, type, ajaxed, options) {
            ...
            else
            {
                ...
                $("#unique"+field.attr("id")).remove();
                field.parent().append('<div class="formErrorSelf" id="unique'+field.attr("id")+'">'+promptText+'</div>');
            }
            //$("body").append(prompt);//--edited by satish kauthale
