...
else if($_REQUEST['action'] == 'add_SubService_media_vidio')
{
	 $arr['service_id']=$_REQUEST['service_id'];
 ...
 ?>
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
        <h4 class="modal-title"> Add Photo & Vidio's</h4>
    </div>
    <div class="modal-body">
        <form class="form-inline" enctype="multipart/form-data" method="post" action="<?php echo $siteURL ;?>/admin/services_ajax_process.php" id="fileUploadForm">
            <input type="hidden" name="service_id" id="service_id" value="<?php echo $arr['service_id']; ?>" />
            <input type="hidden" name="sub_service_id" id="sub_service_id" value="<?php echo $arr['sub_service_id']; ?>" />
			<input type="hidden" name="action" value="add_vidio" />
            ...
            <div class="scrollbars" >                
				...
				<div class="editform" style="width:500px">
					...
					<select name="Service_day" id="Service_day" onchange="select_day();" class="validate[required,maxSize[50]] form-control" maxlength="50"  style="width:100% !important;" onchange="change_type_of_payment();">
				...
				<input type="hidden" name="Service_select" id="Service_select" value="<?php echo $arr['Service_day']; ?>" />
                <div class="editform" style="width:500px">
                    <label>Vidio</label>
                     <div class="value">
						<input type="file"  name="Vidio" id="Vidio" value="<?php echo $Image_name;?>">
						
						<input style="margin-top:10px;"type="Submit" class="btn btn-download" value="submit" />
					</div> 
...
else if($_REQUEST['action'] == 'add_vidio')
{
	//echo "hi";
	...
	$arr['Service_select']=$_REQUEST['Service_select'];
	//echo $arr['sub_service_id'];
 $target_dir =  "/home/spero/public_html/spero_global/admin/images/photo/";
	$target_file = $target_dir . basename($_FILES["Vidio"]["name"]);
	$uploadOk = 1;
	...
	{
		//var_dump($_FILES["Vidio"]["tmp_name"], $target_file);
		if (move_uploaded_file($_FILES["Vidio"]["tmp_name"], $target_file))
		{
			echo "The file ". basename( $_FILES["Vidio"]["name"]). " has been uploaded.";
		} 
		else 
		{
    ...
    $arr['service_id']=$arr['service_id'];
    $arr['sub_service_id']=$arr['sub_service_id'];
    $arr['Service_day']=$arr['Service_select'];
	$arr['Type']='1';
    $arr['path']=$target_file;
     $InsertRecord=$adminClass->InsertPhoto($arr); 
	 $url = $siteURL.'/admin/manage-services.php';
//	 echo $url;
echo "<script>location.href='".$url."';</script>";
}
else if($_REQUEST['action'] == 'vw_upload_service_photo')
{
	 $arr['service_id']=$_REQUEST['service_id'];
    $arr['sub_service_id']=$_REQUEST['subservice_id'];
    $recListResponse1= $adminClass->GetSubServiceById($arr);
 ...
 ?>
    ...
    <div class="modal-body">
        <form class="form-inline" enctype="multipart/form-data" method="post" action="<?php echo $siteURL ;?>/admin/services_ajax_process.php?action=upload_service_photo" name="FrmUploadServicePhoto" id="FrmUploadServicePhoto" autocomplete="off">
            <input type="hidden" name="service_id" id="service_id" value="<?php echo $arr['service_id']; ?>" />
            <input type="hidden" name="sub_service_id" id="sub_service_id" value="<?php echo $arr['sub_service_id']; ?>" />
            ...
            <div class="scrollbars" >                
                ...
                <div class="editform" style="width:500px">
                    <label>Photo</label>
                     <div class="value">
						<input type="file"  name="Photo" id="Photo" value="<?php echo $Image_name;?>">
					</div> 
                </div>
             </div>
			 <div class="modal-footer">
				<input type="button" name="btn_upload_photo" id="btn_upload_photo" class="btn btn-download" value="Upload photo" onclick="add_service_photo(' . $arr['service_id'] . ', ' . $arr['sub_service_id'] . ');" />
			 </div>
        </form>
    </div>
 <?php  
        ...
        <form class="form-inline" name="frm_sub_services" id="frm_sub_services" method="post" action ="services_ajax_process.php?action=inserted_sub_services" autocomplete="off">
            ...
            <div class="scrollbars" >                
                ...
                <div class="editform" style="width:500px">
                    <label>UOM</label>
                    <div class="value">
                        <select name="uom" id="uom" class="validate[required] form-control" style="width:100% !important;">
                            <option value=""<?php if ($recListResponse1['UOM'] == '') { echo 'selected="selected"'; } ?>>Select</option>
                            <option value="Service"<?php if ($recListResponse1['UOM'] == 'Service') { echo 'selected="selected"'; } ?>>Service</option>
                            <option value="day"<?php if ($recListResponse1['UOM'] == 'day') { echo 'selected="selected"'; } ?>>Day</option>
                            <option value="Other"<?php if ($recListResponse1['UOM'] == 'Other') { echo 'selected="selected"'; } ?>>Other</option>	
                        </select>
                    </div>
                </div>
...
} else if($_REQUEST['action'] == 'upload_service_photo') {
	$arr['service_id']=$_REQUEST['service_id'];
	$arr['sub_service_id']=$_REQUEST['sub_service_id'];
	$arr['Service_select']=$_REQUEST['Service_select'];
	$target_dir =  "/home/spero/public_html/spero_global/admin/images/photo/";
