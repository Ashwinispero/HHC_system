...
if($_REQUEST['action']=='vw_add_system_user')
{
    ...
    // Get all assigned hospital
    $assignedHospitalDtls = $adminuserClass->getAssignedHospitalById($arr);
            ...
            <div class="scrollbars">
               ...
               <!-- <div class="editform">
                ...
                <div class="editform">
                    ...
                    <div class="value">
                        <input type="password" name="password" id="password" value="<?php if(!empty($_POST['password'])) { echo $_POST['password']; } else { echo ""; } ?>" class="validate[required,custom[passwordVal],minSize[8],maxSize[25]] form-control" maxlength="25" style="width:100% !important;" />
                    </div>
                </div>
                <div class="editform">
                    <label>Confirm Password <span class="required">*</span></label>
                    <div class="value">
                        <input type="password" name="confirm_password" id="confirm_password" value="<?php if(!empty($_POST['confirm_password'])) { echo $_POST['confirm_password']; } else { echo ""; } ?>" class="validate[required,custom[passwordVal],minSize[8],equals[password],maxSize[25]] form-control" maxlength="25" style="width:100% !important;" />
                    </div>
                </div>
                <?php } ?>
                <div class="editform">
                ...
                <div class="row">
                     <div class="col-lg-12">
                        <?php
                            // echo '<pre>assignedHospitalDtls ----- <br>';
                            // print_r($hospitalList);
                            // echo '</pre>';

                            $hospitalCount = array();
                            $j = 0;
                            foreach ($assignedHospitalDtls AS $allSelectedHospital) {
                                $allhospitalval .= $allSelectedHospital['hospital_id'];
                                $hospitalCount[$j] = $allSelectedHospital['hospital_id'];
                                $j++; 
                            }

                            // echo '<pre>hospitalCount ----- <br>';
                            // print_r($hospitalCount);
                            // echo '</pre>';
                            // exit;

                            foreach($hospitalList AS $allHospitals) {
                                 $cssClass = ''; 
                                 for($h = 0; $h <= count($assignedHospitalDtls); $h++)
                                 {
                                     if ($allHospitals['hospital_id'] == $hospitalCount[$h])
                                        $cssClass = 'checked="checked"';                 
                                 }
                                 echo '<div class="col-md-6">';
                                        if ($cssClass != '')
                                            echo '<input ' . $cssClass . ' type = "checkbox" name = "hospital_name[]" id = "module_name" value = "' . $allHospitals['hospital_id'] . '" />';                                                                    
                                        else
                                            echo '<input type = "checkbox" name = "hospital_name[]" id = "hospital_name" value = "' . $allHospitals['hospital_id'] . '" />';

                                      echo '<label style = "margin-left:10px;">' . $allHospitals['hospital_name'] . '</label>';
                                echo '</div>';
                            }
                        ?>
                     </div>
        ...
        else 
        {
           ...
           // $arr['dob']=$dob;
            ...
            if(!empty($InsertRecord))
            {
                ...
                // Assign hospital
                $totalhospitals = $_POST['hospital_name'];

                if ($admin_user_id && $totalhospitals) {
                    // Delete All assigned hospitals first of this user
                    $delAsssignedHospitalSql = "DELETE FROM sp_admin_user_hospital_mapping WHERE admin_user_id = '" . $admin_user_id . "'";
                    $db->query($delAsssignedHospitalSql);
                }

                $assignedHospitalarr['last_modified_by'] = strip_tags($_SESSION['admin_user_id']);
                $assignedHospitalarr['last_modified_date'] = date('Y-m-d H:i:s');
                
                if($admin_user_id)
                    $assignedHospitalarr['admin_user_id'] = $admin_user_id;
                else 
                    $assignedHospitalarr['admin_user_id'] = $InsertRecord;

                for ($a = 0; $a < count($totalhospitals); $a++) {
                   $assignedHospitalarr['hospital_id'] = $totalhospitals[$a];
                   $assignedHospitalRecordId = $adminuserClass->assignHospitalToUser($assignedHospitalarr); 
                }
