...
//require("config.php");
//$db = new AbstractDB($config['server'],$config['user'],$config['pass'],$config['database'],$config['tablePrefix']);
...
#-#############################################
# desc: constructor
function AbstractDB()//$server, $user, $pass, $database, $pre='')
{
	$this->server=$GLOBALS["host"];
	$this->user=$GLOBALS["dbuid"];
	$this->pass=$GLOBALS["dbpwd"];
	$this->database=$GLOBALS["dbname"];
        
        $this->pre=$GLOBALS["pre_db"];
...
function connect($arg=NULL,$new_link=false) {    
        if(isset($arg))
        {
            $this->link_id=@mysql_connect($arg['host'],$arg['dbuid'],$arg['dbpwd'],$new_link);

            if (!$this->link_id) {//open failed
                    $this->oops("Could not connect to server: <b>".$arg['host']."</b>.");
                    }

            if(!@mysql_select_db($arg['dbname'], $this->link_id)) {//no database
                    $this->oops("Could not open database: <b>".$arg['dbname']."</b>.");
                    }
        }
        else
        {
...
function query_update($table, $data, $where='1') {
	...
	foreach($data as $key=>$val) {
		...
		else $q.= "`$key`='".$this->escape($val,ENT_QUOTES)."', ";
	}
...
function query_insert($table, $data) {
	...
	foreach($data as $key=>$val) {
		...
		else $v.= "'".$this->escape($val,ENT_QUOTES)."', ";
	}
...
function oops($msg='') {
    ...
    ?><script type="text/javascript">//window.location.href='error.php';</script><?php
	?>
