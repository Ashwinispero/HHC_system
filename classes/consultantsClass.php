...
class consultantsClass extends AbstractDB 
{
    ...
    /**
     *
     * This function is used fo add  consultant details
     *
     * @param array $arg
     *
     * @return int $RecordId
     *
     */
    public function RejectConsultant($arg){
        $doctors_consultants_id = $this->escape($arg['doctors_consultants_id']);
        ...
        if ($this->num_of_rows($this->query($ChkConsultantSql)) == 0) {
            ...
            $insertData['reject_by'] = $this->escape($arg['reject_by']);
            $insertData['reject_reason'] = $this->escape($arg['reject_reason']);
            $insertData['status'] = '4';
    ...
    public function ApprovedConsultant($arg){
        ...
        if ($this->num_of_rows($this->query($ChkConsultantSql)) == 0) {
           ...
           // $insertData['middle_name']        = $this->escape($arg['middle_name']);
            ...
            $insertData['last_modified_by']   = $this->escape($arg['last_modified_by']);
            $insertData['last_modified_date'] = $this->escape($arg['last_modified_date']);
            $insertData['approved_by'] = $this->escape($arg['approved_by']);
            $insertData['status'] = '1';
            if (!empty($doctors_consultants_id)) {
                $where = "doctors_consultants_id = '" . $doctors_consultants_id . "'";
                $RecordId = $this->query_update('sp_doctors_consultants', $insertData, $where); 
    ...
    /**
     *
     * This function is used fo update consultant status
     *
     * @param array $arg
     *
     * @return int 0|1
    ...
    public function ChangeStatus($arg)
    {
        ...
        $ChkConsultantSql = "SELECT doctors_consultants_id,
            status,
            isDelStatus,
            last_modified_by,
            last_modified_date
        FROM sp_doctors_consultants 
        WHERE doctors_consultants_id = '" . $doctors_consultants_id . "'";

        if ($this->num_of_rows($this->query($ChkConsultantSql))) {
            $consultantDtls = $this->fetch_array($this->query($ChkConsultantSql));
            if ($istrashDelete) {
            ...
            if (!empty($RecordId) && !empty($consultantDtls)) {
                $insertActivityArr = array();
                $insertActivityArr['module_type']   = '2';
                $insertActivityArr['module_id']     = '7';
                $insertActivityArr['module_name']   = 'Manage Consultants';
                ...
                $activityDesc = "Consultant details modified successfully by " . $_SESSION['admin_user_name'] . "\r\n";

                $activityDesc .= "status is change from " . $consultantDtls['status'] . " to " . $status . "\r\n";
                $activityDesc .= "isDelStatus is change from " . $consultantDtls['isDelStatus'] . " to " . $pre_status . "\r\n";
                $activityDesc .= "last_modified_by is change from " . $consultantDtls['last_modified_by'] . " to " . $login_user_id . "\r\n";
                $activityDesc .= "last_modified_date is change from " . $consultantDtls['last_modified_date'] . " to " . date('Y-m-d H:i:s') . "\r\n";
    ...
    public function addActivity($args)
    {
        $recordId = 0;
        if (!empty($args)) {
            $consultantId   = $args['doctors_consultants_id'] ;
            $consultantDtls = $args['consultant_dtls'];
            ...
            $activityDesc = "Consultants details " . ( $consultantId ? 'modified' : 'added' ) . " successfully by " . $_SESSION['admin_user_name'] . "\r\n";

            if (!empty($consultantDtls) && !empty($insertData)) {
                unset($consultantDtls['doctors_consultants_id'],
                    $consultantDtls['status'],
                    $consultantDtls['statusVal'],
                    $consultantDtls['typeVal'],
                    $consultantDtls['added_by'],
                    $consultantDtls['added_date'],
                    $consultantDtls['last_modified_by']
                );
                $consultantDiff = array_diff_assoc($consultantDtls, $insertData);
                if (!empty($consultantDiff)) {
                    foreach ($consultantDtls AS $key => $valConsultant) {
                        $activityDesc .= $key . " is change from " . $valConsultant . " to " . $insertData[$key] . "\r\n";
                    }
                }
            }
            $insertActivityArr['activity_description'] = (!empty($activityDesc) ? nl2br($activityDesc) : "");
