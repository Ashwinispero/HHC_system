...
<?php
  ...
  $flag=$_GET['flag'];
	...
	$HCA = mysql_query("SELECT * FROM `sp_professional_services` where service_id='4' and status='1' ");
	...
	if($row_count > 0)
		...
		$datas .=  '<div align="center">
					...
					<span><b>HCA Scheduling</span></b><br>
					</div>';
	...
	date_default_timezone_set('Asia/Kolkata'); 
        ...
        $datas .=  '<table cellpadding="1" cellspacing="1" align="left" border="1" id="mainTableBg">
                        <tr height="30">
							 <td width="3%" align="center">HCA Name</td>
							 <td width="5%" align="center">Preferd location</td>
        ...
        while ($HCA_rows = mysql_fetch_array($HCA))
		{
			...
			while ($rows = mysql_fetch_array($event_professional))
			{
				$event_requirement_id=$rows['event_requirement_id'];
					...
					{
						$day1='<span style="color:green">Available</span>';
					$day2='<span class="glyphicon glyphicon-ok" style="color:green">Available</span>';
					$day3='<span class="glyphicon glyphicon-ok" style="color:green">Available</span>';
					$day4='<span class="glyphicon glyphicon-ok" style="color:green">Available</span>';
					$day5='<span class="glyphicon glyphicon-ok" style="color:green">Available</span>';
...
include "include/paging_script.php";
    ...
    $db->close();
    //echo $csv;
    $filepath="CSV/".time()."HCA_Schedule.xls";
	//echo $filepath;exit;
    $file=fopen($filepath,"w");
    fwrite($file,$datas);
    fclose($file);
    header("Content-Disposition: attachment; filename=HCA_Schedule".date("Y-m-d").".xls");
