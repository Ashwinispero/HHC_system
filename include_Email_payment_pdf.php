<?php
    ...
    $event_id=$db->escape($_REQUEST['eventid']);
...
<html>
                            ...
                            <tr>
                                <td align="left"> 
                                    <div style="float:left;">
                                        <h3 style="color:#00cfcb; font-size:15px; vertical-align:middle; font-weight:normal; color:#000;"> Receipt  <span style="float:right;padding-right: 25px;"><br/> <?php echo "Event:".$event_code; ?>     |  <?php if(!empty($EventSummaryDtls['PatientDtls']['hhc_code'])) { echo  "HHC No. :".$EventSummaryDtls['PatientDtls']['hhc_code']; } ?></span></h3>
                                    </div>
                                    <div style="float:left;">
	...
	$sql1=mysql_query("SELECT * FROM sp_event_requirements  where event_id='$event_number'");
			...
			if($row_count > 0)
			{
				...
				$professional_name=$title.' '.$Pname.' '.$Pfirst_name;
				}
				else
				{
				$professional_name='';
...
function convert_number_to_words($number) {
    ...
    return $string;
            ...
            ?>
			<div style="background:#e4e1e1; height:1px; width:100%; margin-top:20px; margin-bottom:10px;"></div>
			...
			if($Transaction_Type=='Payment')
			{
				$Amount1=$amount;
					...
					<?php	
						...
						$payment_detail=mysql_query("SELECT Max(payment_id) as payment_id FROM sp_payments") or die(mysql_error());
						$payment_details = mysql_fetch_array($payment_detail) or die(mysql_error());
						...
						$amount=$payment_details_row['amount'];
						$type=$payment_details_row['type'];
						$date_time=$payment_details_row['date_time'];
						$Card_Number=$payment_details_row['Card_Number'];
						$Transaction_ID=$payment_details_row['Transaction_ID'];
				...
				<?php if(!empty($EventSummaryDtls['PatientDtls']['name'])) { echo $EventSummaryDtls['PatientDtls']['name']." "; } if(!empty($EventSummaryDtls['PatientDtls']['first_name'])) { echo $EventSummaryDtls['PatientDtls']['first_name']." "; } if(!empty($EventSummaryDtls['PatientDtls']['middle_name'])) { echo $EventSummaryDtls['PatientDtls']['middle_name']; }  else {  echo ""; } ?></div>			 
							  ...
							  <thead>
								<tr style=" padding:3px;">
                                  ...
                                  <th style="text-align:left;font-weight:normal;width:50%;">Rupees:<?php  echo $in_words; ?></th>
			...
			<form class="form-horizontal rounded-corner col-lg-12">
					...
					<div class="col-lg-12" >Shiv Corner, Samarth Park - Brahma Garden Rd, Anand Nagar, Pune, Maharashtra 411051, Email : Info@sperohealthcare.in, Website:WWW.Sperohealthcare.in.</div>
				<div class="col-lg-12" ><b>For Future Services Contact No.:</b>7620400100</div>
					<div class="col-lg-12" ><b>In case of any concern / suggestion :</b> Escalation Contact No.1 : 7774091977 | Escalation Contact No.2 : 7774081894 </div>
